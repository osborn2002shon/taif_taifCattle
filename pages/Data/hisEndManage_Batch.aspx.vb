...
Public Class hisEndManage_Batch
    ...
    Private Sub LinkButton_downloadFailed_Click(sender As Object, e As EventArgs) Handles LinkButton_downloadFailed.Click
        ...
        Dim headers As String() = {"牛籍編號", "除籍日期", "類型", "畜牧場證號", "除籍備註", "失敗原因"}
    ...
    Private Sub ProcessImport(excelData As DataTable)
        ...
        For Each row As DataRow In excelData.Rows
            ...
            Dim dataDateText As String = Convert.ToString(row("除籍日期")).Trim()
            ...
            Dim needNewCattle As Boolean = False
            ...
            If String.IsNullOrEmpty(farmCode) Then
                reasons.Add("畜牧場錯誤")
            ElseIf Not farmLookup.TryGetValue(farmCode, farmId) Then
                reasons.Add("畜牧場錯誤")
            End If
            ...
            Dim insertedNewCattle As Boolean = False
            Dim preHistoryId As Integer = 0

            Try
                If needNewCattle Then
                    Dim cattleInfo As New taifCattle.Cattle.stru_cattleInfo With {
                        .cattleTypeID = 6,
                        .tagNo = tagNo,
                        .tagMemo = Nothing,
                        .birthYear = DBNull.Value,
                        .cattleMemo = Nothing,
                    ...
                    insertedNewCattle = True
                End If

                If dataDateValue > DateTime.MinValue Then
                    Dim previousDay As DateTime = dataDateValue.AddDays(-1)
                    If Not taifCattle_cattle.Check_IsHistoryExist(cattleId, previousDay, farmId) Then
                        Dim preHistory As New taifCattle.Cattle.stru_cattleHistory With {
                            .cattleID = cattleId,
                            .hisTypeID = 2,
                            .dataDate = previousDay,
                            .farmID = farmId,
                            .plantID = Nothing,
                            .slauID = Nothing,
                            .memo = Nothing,
                        ...
                        preHistoryId = Convert.ToInt32(taifCattle_cattle.Insert_CattleHistory(preHistory))
                    End If
                End If
                ...
                Insert_UserLog(insertUserId, enum_UserLogItem.除籍批次設定功能, enum_UserLogType.新增, $"hisID:{hisId}")
            Catch ex As Exception
                If preHistoryId > 0 Then
                    DeleteHistoryById(preHistoryId)
                End If

                If insertedNewCattle AndAlso cattleId > 0 Then
    ...
    Private Function GetHistoryTypeLookup() As Dictionary(Of String, Integer)
        ...
        Dim allowedTypes As New HashSet(Of String)(StringComparer.OrdinalIgnoreCase) From {"其他", "未使用"}
        ...
        Using da As New DataAccess.MS_SQL()
            ...
            Dim para As New SqlParameter("groupName", "除籍")
            Dim dt As DataTable = da.GetDataTable(sql, para)
            For Each row As DataRow In dt.Rows
                Dim name As String = Convert.ToString(row("typeName")).Trim()
                If allowedTypes.Contains(name) AndAlso Not dict.ContainsKey(name) Then
